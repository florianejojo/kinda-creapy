...
const reduceImageQuality = (inputPath, quality = 50) => {
  ...
  const command = `magick "${inputPath}" -resize 800x800\\> -quality ${quality} "${inputPath}"`

  exec(command, (error, stdout, stderr) => {
    if (error) {
      console.error(`Erreur lors de la réduction de qualité : ${error.message}`)
      return
    }
...
processImagesInDirectory(directoryPath, 100) // Réduit la qualité à 30%
