        ...
        files.forEach((file) => {
            ...
            if ([".heic", ".png", ".jpg", ".jpeg", ".tiff"].includes(ext)) {
                ...
                if (!fs.existsSync(webpFilePath)) {
                    ...
                    console.log(
                        `Conversion de ${originalFilePath} en ${webpFilePath}`
                    );

                    // Utilisation d'ImageMagick pour la conversion
                    exec(
                        `magick "${originalFilePath}" "${webpFilePath}"`,
                        (error, stdout, stderr) => {
                            if (error) {
                                console.error(
                            ...
                            console.log(
                                `Fichier converti avec succès: ${webpFilePath}`
                            );
                        }
                    );
                } else {
                    console.log(
                        `Le fichier ${webpFilePath} existe déjà, conversion ignorée.`
                    );
                }
            }
        });
